...
int main(int argc, char *argv[]){
	...
	MT_WIFI_PATCH_MMAP			= (unsigned char *)mmap(
        ...
        memset(MT_WIFI_PATCH_MMAP, 	0x00, 		   MT_WIFI_PATCH_SIZE);
	memcpy(MT_WIFI_PATCH_MMAP, MT_WIFI_PATCH_MMAP_TMP, MT_WIFI_PATCH_SIZE);
	...
	printf("> insert the new region\'s size: ");
	scanf("%d", &new_region_patch_size);
	printf("segment %d will be loaded at address: 0x%x with size: %d\n", MT_WIFI_PATCH_HDR->desc.n_region, address_to_patch_addr, new_region_patch_size);
	// create the new region

	new_region_patch_data				= (unsigned char *)malloc(new_region_patch_size);
	memset(new_region_patch_data,			'A',		new_region_patch_size);

	// create the new patch file

	unsigned int final_patch_file_size		= MT_WIFI_PATCH_SIZE   +
                                                          sizeof(MT_PATCH_SEC) +
                                                          new_region_patch_size;


	unsigned char *final_patch 			= (unsigned char *)mmap(
								NULL,
								final_patch_file_size,
								PROT_READ | PROT_WRITE,
								MAP_ANON  | MAP_PRIVATE,
								-1,
								0x00
	...
	memset(final_patch, 	0x00, 			final_patch_file_size);
	// copy the header
	MT_PATCH_HDR *FINAL_WIFI_PATCH			= (MT_PATCH_HDR *)final_patch;
	//memcpy(final_patch,         			MT_WIFI_PATCH_MMAP,      sizeof(*MT_WIFI_PATCH_HDR));
	memcpy(FINAL_WIFI_PATCH->build_date,		MT_WIFI_PATCH_HDR->build_date,	 MT_BUILD_DATE_LEN);
	memcpy(FINAL_WIFI_PATCH->platform, 		MT_WIFI_PATCH_HDR->platform,	 MT_PLATFORM_LEN);
	FINAL_WIFI_PATCH->hw_sw_ver			= __be32_to_cpu(MT_WIFI_PATCH_HDR->hw_sw_ver);
	FINAL_WIFI_PATCH->patch_ver                     = __be32_to_cpu(MT_WIFI_PATCH_HDR->patch_ver);
	FINAL_WIFI_PATCH->checksum			= __be32_to_cpu(MT_WIFI_PATCH_HDR->checksum);
	FINAL_WIFI_PATCH->rsv                           = __be32_to_cpu(MT_WIFI_PATCH_HDR->rsv);
	FINAL_WIFI_PATCH->desc.patch_ver		= __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.patch_ver);
	FINAL_WIFI_PATCH->desc.subsys			= __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.subsys);
	FINAL_WIFI_PATCH->desc.feature			= __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.feature);
	FINAL_WIFI_PATCH->desc.n_region			= __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.n_region) + 1;
	FINAL_WIFI_PATCH->desc.crc			= __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.crc);

	unsigned int final_patch_offset_region_start	= 0x00;

        for(unsigned char j = 0; j < __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.n_region); j++){
                MT_PATCH_SEC *MT_WIFI_PATCH_CURR_SECT   = NULL;

                MT_WIFI_PATCH_CURR_SECT                 = (MT_PATCH_SEC *)(MT_WIFI_PATCH_MMAP + sizeof(*MT_WIFI_PATCH_HDR) + j * sizeof(*MT_WIFI_PATCH_CURR_SECT));
		MT_WIFI_PATCH_CURR_SECT->offs	       += sizeof(MT_PATCH_SEC);

		memcpy(final_patch + sizeof(MT_WIFI_PATCH_HDR) + j * sizeof(MT_PATCH_SEC), MT_WIFI_PATCH_CURR_SECT, sizeof(MT_PATCH_SEC));
		final_patch_offset_region_start		= MT_WIFI_PATCH_CURR_SECT->offs + __be32_to_cpu(MT_WIFI_PATCH_CURR_SECT->info.len);
	}

	// now add a new region
	MT_PATCH_SEC MT_WIFI_PATCH_NEW_REGION           = { 0 };
	memset(&MT_WIFI_PATCH_NEW_REGION,	0x00,	sizeof(MT_PATCH_SEC));
 	MT_WIFI_PATCH_NEW_REGION.info.addr		= (void *)address_to_patch_addr;
        MT_WIFI_PATCH_NEW_REGION.info.len		= (int)new_region_patch_size;
        //memcpy(&MT_WIFI_PATCH_NEW_REGION.info.sec_key_idx, &MT_WIFI_PATCH_CURR_SECT->info.sec_key_idx, siz);
        MT_WIFI_PATCH_NEW_REGION.offs			= final_patch_offset_region_start;

	memcpy(final_patch + sizeof(MT_WIFI_PATCH_HDR) + __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.n_region) * sizeof(MT_PATCH_SEC), &MT_WIFI_PATCH_NEW_REGION, sizeof(MT_PATCH_SEC));
	memcpy(final_patch + MT_WIFI_PATCH_SIZE,										 new_region_patch_data,	    new_region_patch_size);

	munmap(MT_WIFI_PATCH_MMAP, MT_WIFI_PATCH_SIZE);
	final_patch_fd					= open("final_patch", O_CREAT | O_RDWR, 0777);
	write(final_patch_fd, final_patch,		final_patch_file_size);
	close(final_patch_fd);
	munmap(final_patch, 				final_patch_file_size);
	return 0;
}
