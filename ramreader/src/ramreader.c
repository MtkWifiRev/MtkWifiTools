/**
        ...
        Simple RAM reader which permits to read live RAM with the objdump's NDS32 disassembly
**/
...
static signed int  MT76_DISASSEMBLE_with_bytes_CODE(unsigned char  *MT76_REGVAL_BYTES, unsigned int MT76_REGVAL_LEN, unsigned long REGVAL_ADDR){
...
static signed int disasm_ram(long ADDR, long mem_range, unsigned char REGIDX_PATH[], unsigned char REGVAL_PATH[]){
        ...
        MT76_DISASSEMBLE_with_bytes_CODE(current_memory_range, mem_range, ADDR);
...
static unsigned int length_align(unsigned int UNALIGN_LEN){
	/** if the length is 0x00, set to the minimun length for disassembling at least 2 32-bit instructions (8 bytes) **/
	if( UNALIGN_LEN == 0x00 ){
		UNALIGN_LEN	= 0x8;
	}

	return (UNALIGN_LEN + (0x4 - 1)) & ~(0x4 - 1);
}
...
int main(){
	...
	unsigned int  ALIGNED_LEN;
        ...
        while( 1 ){
			...
			printf("insert the length of the data you want to read:\n");
			printf("> ");
			scanf("%u", &LEN);
			ALIGNED_LEN	= length_align(LEN);
			#ifdef DEBUG
			printf("aligned len: %u\n", ALIGNED_LEN);
                        #endif
			break;
                }
        }

        disasm_ram(ADDR, ALIGNED_LEN, REGIDX_PATH, REGVAL_PATH);
	return 0;
}
