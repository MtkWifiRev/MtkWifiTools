/**
	...
	NOTE: this is a special version for mt7921 (and probably mt7922) ONLY!
	it adds a padding region of 1068 bytes BEFORE the start of the second region (the .data region)
**/
...
int main(int argc, char* argv[]) {
	...
	**/
	...
	/**
		...
		--
		EDIT:
		Initialize the total size of the segments with the 1068 bytes of padding from now
	**/

	unsigned int final_elf_segment_total_number	= 0;
	unsigned int final_elf_segment_total_size	= MT7961_PADDING;
	...
	**/
	...
	/** start the main loop **/
		...
		const struct mt76_connac2_fw_trailer *hdr = (const struct mt76_connac2_fw_trailer *)(
       	        ...
       	        printf(GREEN "[*] " CRESET "header at 0x%x\n", CANDIDATE_NEW_ELEM->current_metadata.candidate_file_memory_map  +
                                                               CANDIDATE_NEW_ELEM->current_metadata.candidate_file_size                -
	...
	unsigned char MT7961_PAD[MT7961_PADDING]= {};
	memset(MT7961_PAD, 0x00, MT7961_PADDING);
	...
	while (CANDIDATE_FW_CURRENT != NULL) {
		...
		for(unsigned int k = 0; k < hdr->n_region; k++){
			...
			if( !( region->feature_set & FW_FEATURE_NON_DL ) ){
				...
				if ( k == 1 ){
					CURR_PHDR->p_filesz	= len + MT7961_PADDING;
					CURR_PHDR->p_memsz	= len + MT7961_PADDING;
				}else{
				...
				/**
					apply this for the mt7961 data region
				**/
				if( k == 1 ){
					memmove(
					...
					memmove(
						ELF_HDR + PHDR_ACTUAL_OFFSET + MT7961_PADDING,
                                                CANDIDATE_FW_CURRENT->current_metadata.candidate_file_memory_map +
	...
	mtk_fw_final_elf                = open("mt7961.elf", O_CREAT | O_RDWR, 0777);
