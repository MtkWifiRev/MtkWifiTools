...
int main(int argc, char *argv[]){
	...
	int		 MT_WIFI_PATCH_COUNTER_NMBR	= 0x00;
	...
	unsigned int metadata_end_offset = sizeof(*MT_WIFI_PATCH_HDR) + __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.n_region) * sizeof(MT_PATCH_SEC);
	int fd = open("patch_hdr.bin", O_CREAT | O_RDWR, 0777);
	write(fd, MT_WIFI_PATCH_MMAP, metadata_end_offset);
	close(fd);
	for(unsigned char j = 0; j < __be32_to_cpu(MT_WIFI_PATCH_HDR->desc.n_region); j++){
		...
		// split it into various files
		unsigned char file_name[16];
		memset(file_name, ' ', 16);
		snprintf(file_name, 16, "region_%d", j);
		int file_splitted_fd = open(file_name, O_CREAT | O_RDWR, 0777);
		write(file_splitted_fd, dl, __be32_to_cpu(MT_WIFI_PATCH_CURR_SECT->size));
		close(file_splitted_fd);
